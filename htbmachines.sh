...
function getYoutubeLink(){
  ...
  if [ "$youtubeLink" ]; then
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} El tutorial para esta máquina '${endColour}${purpleColour}$machineName${endColour}${grayColour}' está en el siguiente enlace: ${blueColour}$youtubeLink${endColour}\n"
  else
    echo -e "\n${redColour}[!] La máquina proporcionada no existe${endColour}\n"
  fi
}
...
function getMachinesDifficulty(){
  ...
  results_check="$(cat bundle.js| grep "dificultad: \"Media\"" -B 5 | grep "name:" | awk 'NF{print $NF}' | tr -d '",' | column)"
...
function getOSMachines(){
  ...
  os_results="$(cat bundle.js | grep "so: \"$os\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column)"
  ...
  cat bundle.js | grep "so: \"$os\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column
  else
...
function getOSDifficultyMachines(){
  ...
  cat bundle.js | grep "so: \"$os\"" -C 4 | grep "dificultad: \"$difficulty\"" -B 5 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column
  else
...
function getSkill(){
  ...
  if [ "$check_skill" ]; then
    ...
    echo -e "\n${yellowColour}[+]${endColour}${grayColour} A continuación se representan las máquinas que utilizan la Skill${endColour}${blueColour} $skill${endColour}${grayColour}:${endColour}\n"
  cat bundle.js | grep "skills: " -B 6 | grep "$skill" -i -B 6 | grep "name: " | awk 'NF{print $NF}' | tr -d '",' | column
  else 
    echo -e "\n${redColour}[!] NO se han encontrado ninguna máquina con la Skill indicada${endColour}\n"
  fi
}
